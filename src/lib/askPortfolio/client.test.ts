...
describe('streamChat', () => {
  it('mints a token, posts the messages, and yields parsed events', async () => {
    ...
    const events = await collect(
      streamChat({ messages, sessionId: 'sess-1', getToken, apiBaseUrl: 'https://api.test', fetchImpl }),
    );
    expect(getToken).toHaveBeenCalledOnce();
    const [url, init] = fetchImpl.mock.calls[0];
    expect(url).toBe('https://api.test/api/chat');
    expect(JSON.parse(init.body)).toEqual({ messages, turnstileToken: 'tok', sessionId: 'sess-1' });
  ...
  it('yields a turnstile error on 403', async () => {
    ...
    const events = await collect(
      streamChat({ messages, sessionId: 'sess-1', getToken: async () => 'tok', apiBaseUrl: 'https://api.test', fetchImpl }),
    );
